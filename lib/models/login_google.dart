...
import 'package:buyayak/home_page.dart';
import 'package:buyayak/halamandaftar.dart'; // Pastikan import halaman daftar kamu

class LoginGoogleService {
  static Future<void> prosesLoginGoogle(BuildContext context, Function(String, Color) showMessage) async {
    showMessage("Menghubungkan ke Google...", Colors.blue);

    // Simulasi email dari Google Account
    String googleEmail = "eve.holt@reqres.in"; 
    String passwordOtomatis = "pistol"; // Diubah jadi 'pistol' agar Reqres tidak mengembalikan error missing password/api

    // ALUR 1: Cek apakah akun Google ini sudah pernah terdaftar di memori lokal
    if (googleEmail == UserData.registeredEmail) {
